...
function App() {
  ...
  const navigateTo = (nextPage: Page) => { setPage(nextPage); setMenuOpen(false); window.location.hash = pageHref(nextPage, service).slice(1) }
...
function CommandPalette({ page, onNavigate, onClose }: { page: Page; onNavigate: (page: Page) => void; onClose: () => void }) {
  ...
  return createPortal(<div className="command-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section ref={dialog} className="command-dialog" role="dialog" aria-modal="true" aria-labelledby="command-title" onKeyDown={handleKey}><div className="command-search"><Search size={19} aria-hidden="true" /><label className="sr-only" htmlFor="command-input" id="command-title">Go to a workspace page</label><input ref={input} id="command-input" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find a page…" autoComplete="off" /><button type="button" onClick={onClose} aria-label="Close page finder"><X size={17} aria-hidden="true" /></button></div><div className="command-results"><p>Navigate</p>{results.map(([label, Icon]) => <button className={page === label ? 'selected' : ''} key={label} onClick={() => onNavigate(label)}><span><Icon size={17} aria-hidden="true" /><span><strong>{label}</strong><small>Open {label.toLowerCase()} workspace</small></span></span><kbd>Enter</kbd></button>)}{results.length === 0 ? <div className="command-empty"><Search size={24} aria-hidden="true" /><span>No matching pages</span></div> : null}</div><footer><span><kbd>Tab</kbd> Navigate</span><span><kbd>Esc</kbd> Close</span></footer></section></div>, document.body)
}
