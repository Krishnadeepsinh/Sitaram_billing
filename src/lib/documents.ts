...
async function createStatementPdf(variant: StatementVariant, data: StatementData, settings: BusinessSettings, adminName = 'Shaktisinh') {
  ...
  const serviceLabel = data.serviceType === 'broadband' ? 'Broadband Subscription' : 'Digital Cable TV'
  const supportNumber = data.serviceType === 'cable' ? '9377913116' : '9825039825'
  ...
  const drawFooter = (text: string, boldValue?: string) => { page.drawLine({ start: { x: 30, y: 50 }, end: { x: 565, y: 50 }, thickness: .8, color: navy }); if (!boldValue || !text.includes(boldValue)) { drawCenter(text, 297.5, 29, 6.8, false, muted); return }; const before = text.slice(0, text.indexOf(boldValue)); const after = text.slice(text.indexOf(boldValue) + boldValue.length); const totalWidth = width(before, 6.8) + width(boldValue, 6.8, true) + width(after, 6.8); let x = 297.5 - totalWidth / 2; draw(before, x, 29, 6.8, false, muted); x += width(before, 6.8); draw(boldValue, x, 29, 6.8, true, orange); x += width(boldValue, 6.8, true); draw(after, x, 29, 6.8, false, muted) }
  ...
  if (variant === 'invoice') {
    ...
    drawStatusGuide(finalTop - 61); drawFooter(`Computer Generated Receipt - No Signature Required | ${settings.businessName || 'Sitaram Cable & Broadband'} | Support: ${supportNumber}`, supportNumber)
  }
  return pdf.save({ useObjectStreams: false })
}
