...
export type Customer = { id: number; customerCode: string; name: string; phone: string | null; stbNumber: string | null; status: 'active' | 'inactive'; nextBillingStartDate: string | null; installationDate: string | null; areaId: number; planId: number | null; areaName: string; planName: string | null; planPricePaise: number | null; planIsActive: number; amountDuePaise: number; creditBalancePaise: number; openInvoiceCount: number; oldestDuePeriodStart: string | null; latestDuePeriodEnd: string | null; latestPeriodStart: string | null; latestPeriodEnd: string | null; coverageStatus: 'never_billed' | 'future' | 'active' | 'expiring_today' | 'expired'; hasHistoricalGap: number }
export type Invoice = { id: number; invoiceCode: string; customerId: number; customerName: string; periodStart: string; periodEnd: string; issuedDate: string; totalPayablePaise: number; balancePaise: number; status: string; dueDate: string; billingMode: 'normal' | 'historical'; isMerged: number; isCombined: number }
export type InvoiceDetail = Invoice & { customerCode: string; areaName: string; planName: string; stbNumber: string | null; monthsBilled: number; currentPeriodAmountPaise: number; previousDueSnapshotPaise: number; liveBalancePaise: number; currentCustomerDuePaise: number; historicalReason?: string | null; charges: Array<{ chargeType: string; description: string; amountPaise: number }>; allocations: Array<{ paymentCode: string; paymentDate: string; cashPaise: number; discountPaise: number; creditPaise: number }>; mergeItems: Array<{ invoiceCode: string; planName: string; periodStart: string; periodEnd: string; amountPaise: number }> }
...
export type InvoiceDeletePreview = { invoiceCode: string; customerId: number; billingMode: string; periodStart: string; periodEnd: string; currentNextBillingDate: string; payments: Array<{ id: number; paymentCode: string; amountReceivedPaise: number; discountGivenPaise: number }>; affectedInvoices: Array<{ invoiceCode: string }> }
...
export function deleteCustomer(serviceType: ServiceType, id: number, reason: string) { return request<void>(`/api/customers?serviceType=${serviceType}&id=${id}&reason=${encodeURIComponent(reason)}`, { method: 'DELETE' }) }
export function restoreCustomer(serviceType: ServiceType, id: number, reason: string) { return request<void>('/api/customers', { method: 'PATCH', body: JSON.stringify({ serviceType, id, reason }) }) }
...
export function createInvoice(serviceType: ServiceType, input: { customerId: number; monthsBilled: number; expectedPeriodStart: string; periodStart?: string; billingMode?: 'normal' | 'historical'; historicalReason?: string }) { return request<{ invoiceCode: string; periodStart: string; periodEnd: string; nextEligibleDate: string; replayed: boolean }>('/api/invoices', { method: 'POST', body: JSON.stringify({ serviceType, ...input }) }) }
export function bulkCreateInvoices(serviceType: ServiceType, throughMonth: string, customerIds?: number[]) { return request<{ generated: Array<{ invoiceCode: string }>; skipped: Array<{ customerId: number; reason: string }>; failed: Array<{ customerId: number; reason: string }> }>('/api/invoices/bulk', { method: 'POST', body: JSON.stringify({ serviceType, throughMonth, customerIds }) }) }
...
export function createPayment(serviceType: ServiceType, input: { customerId: number; paymentDate: string; amountReceivedPaise: number; discountGivenPaise: number; paymentMode: 'cash' | 'upi'; notes?: string; requestKey: string }) { return request<{ paymentCode: string; replayed?: boolean }>('/api/payments', { method: 'POST', body: JSON.stringify({ serviceType, ...input }) }) }
