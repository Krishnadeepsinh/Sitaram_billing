...
export type Invoice = { id: number; invoiceCode: string; customerId: number; customerName: string; periodStart: string; periodEnd: string; billingMonth: string; issuedDate: string; currentPeriodAmountPaise: number; previousDueSnapshotPaise: number; totalPayablePaise: number; chargeAmountPaise: number; balancePaise: number; status: string; dueDate: string; billingMode: 'normal' | 'historical'; isMerged: number; isCombined: number }
...
export function createInvoice(serviceType: ServiceType, input: { customerId: number; monthsBilled: number; expectedPeriodStart: string; periodStart?: string; issuedDate?: string; billingMonth?: string; billingMode?: 'normal' | 'historical'; historicalReason?: string; restartService?: boolean }) { return request<{ invoiceCode: string; periodStart: string; periodEnd: string; nextEligibleDate: string; replayed: boolean }>('/api/invoices', { method: 'POST', body: JSON.stringify({ serviceType, ...input }) }) }
export type BulkInvoiceResult = { generated: Array<{ invoiceCode: string; customerId: number; customerCode: string; customerName: string; periodStart: string; periodEnd: string; cycles: number; amountPaise: number }>; ready: Array<{ customerId: number; customerCode: string; customerName: string; periodStart: string; periodEnd: string; cycles: number; amountPaise: number }>; skipped: Array<{ customerId: number; customerCode: string; customerName: string; reason: string }>; failed: Array<{ customerId: number; customerCode?: string; customerName?: string; reason: string }> }
export function bulkCreateInvoices(serviceType: ServiceType, throughMonth: string, customerIds?: number[], preview = false, periodStart?: string) { return request<BulkInvoiceResult>('/api/invoices/bulk', { method: 'POST', body: JSON.stringify({ serviceType, throughMonth, customerIds, preview, ...(periodStart ? { periodStart } : {}) }) }) }
