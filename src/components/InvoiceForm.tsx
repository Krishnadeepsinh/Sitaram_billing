...
import { formatBusinessDate, todayInBusinessTimezone } from '../lib/date'
...
export function InvoiceForm({ serviceType, customers, initialCustomerId, onCreated, onCancel }: { serviceType: ServiceType; customers: Customer[]; initialCustomerId?: number; onCreated: (result: { invoiceCode: string; replayed: boolean; periodStart: string; periodEnd: string }) => void; onCancel: () => void }) {
  ...
  return <form className="modal-form single-column invoice-workflow" onSubmit={submit}>
    ...
    <div className="balance-fields"><label>Start Date *<input name="periodStart" type="date" value={startDate} readOnly={mode === 'normal'} onChange={(event) => setStartDate(event.target.value)} required aria-describedby="billing-eligibility" /></label><label>30-Day Cycles *<input name="monthsBilled" type="number" min="1" max="24" value={cycles} onChange={(event) => setCycles(Number(event.target.value))} required /></label></div>
    {preview ? <section className="invoice-preview" aria-live="polite"><div><span>Plan</span><strong>{preview.planName || 'Unavailable'}</strong></div><div><span>Cycle price</span><strong>{formatRupees(preview.pricePaise)}</strong></div><div><span>Service expiry</span><strong>{formatBusinessDate(preview.periodEnd)}</strong></div><div><span>Payment due</span><strong>{formatBusinessDate(preview.dueDate)}</strong></div><div className="preview-total"><span>Total amount</span><strong>{formatRupees(preview.totalPaise)}</strong></div><p id="billing-eligibility" className={preview.conflict ? 'eligibility conflict' : 'eligibility'}><FileCheck2 size={16} aria-hidden="true" />{preview.conflict ? `${preview.conflict.invoiceCode} already covers this range. Choose ${formatBusinessDate(preview.currentNextBillingDate || preview.nextEligibleDate)} or another uncovered date.` : `After this invoice, the next eligible billing date is ${formatBusinessDate(preview.nextEligibleDate)}.`}</p></section> : null}
