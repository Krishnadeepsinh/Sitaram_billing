...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    if (request.method === 'GET') {
      ...
      if (request.query.id) {
        ...
        const payment = await db.execute({ sql: `SELECT payments.id, payments.payment_code AS paymentCode, payments.customer_id AS customerId, payments.customer_code_snapshot AS customerCode,
          payments.customer_name_snapshot AS customerName, payments.stb_number_snapshot AS stbNumber, payments.area_name_snapshot AS areaName, payments.payment_date AS paymentDate,
          ...
          payments.notes, payments.resulting_status AS resultingStatus FROM payments JOIN customers ON customers.id = payments.customer_id
    ...
    const result = await withWriteTransaction(async (transaction) => {
      const existing = await transaction.execute({ sql: 'SELECT payment_code AS paymentCode, resulting_status AS resultingStatus FROM payments WHERE service_type = ? AND request_key = ? LIMIT 1', args: [input.serviceType, input.requestKey] })
      if (existing.rows[0]) return { paymentCode: String(existing.rows[0].paymentCode), resultingStatus: String(existing.rows[0].resultingStatus), allocations: [], replayed: true }
