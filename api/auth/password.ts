...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    await database().execute({ sql: 'DELETE FROM admin_sessions WHERE username = ?', args: [session.username] })
    await setSession(response, session.username)
    return response.status(204).end()
  } catch (error) {
    if (error instanceof z.ZodError) return sendError(response, 400, 'New password must contain at least 10 characters.')
