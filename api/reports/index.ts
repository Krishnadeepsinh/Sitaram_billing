...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    const invoiceDateColumn = dateBasis === 'service' ? 'period_start' : 'issued_date'
    const db = database()
    const [billed, collected, discounts, today, outstanding, subscribers, payments, paymentCount, expenses, expenseCount, trends, quality, modeTotals, expiringSoon, areaBreakdown] = await Promise.all([
      db.execute({ sql: `SELECT COALESCE(SUM(current_period_amount_paise), 0) AS value FROM invoices WHERE is_deleted = 0 AND is_merged = 0 AND ${invoiceDateColumn} BETWEEN ? AND ? AND (? IS NULL OR service_type = ?) AND (? IS NULL OR area_id_snapshot = ?)`, args: [from, to, service, service, areaId, areaId] }),
      ...
      db.execute({ sql: `SELECT COALESCE(SUM(charges.total - COALESCE(allocated.total, 0)), 0) AS value FROM invoices JOIN (SELECT invoice_id, SUM(amount_paise) AS total FROM invoice_charges GROUP BY invoice_id) charges ON charges.invoice_id = invoices.id LEFT JOIN (SELECT invoice_id, SUM(amount_cash_paise + amount_discount_paise + amount_credit_paise) AS total FROM payment_allocations WHERE is_deleted = 0 GROUP BY invoice_id) allocated ON allocated.invoice_id = invoices.id WHERE invoices.is_deleted = 0 AND invoices.is_merged = 0 AND (? IS NULL OR invoices.service_type = ?) AND (? IS NULL OR invoices.area_id_snapshot = ?)`, args: [service, service, areaId, areaId] }),
      ...
      db.execute({ sql: `SELECT month, SUM(billed) AS billedPaise, SUM(collected) AS collectedPaise FROM (
        SELECT substr(${invoiceDateColumn},1,7) AS month, current_period_amount_paise AS billed, 0 AS collected FROM invoices WHERE is_deleted = 0 AND is_merged = 0 AND ${invoiceDateColumn} BETWEEN ? AND ? AND (? IS NULL OR service_type = ?) AND (? IS NULL OR area_id_snapshot = ?)
        ...
        ) GROUP BY month ORDER BY month DESC LIMIT 12`, args: [from, to, service, service, areaId, areaId, from, to, service, service, areaId, areaId, paymentMode, paymentMode, discountGiven] }),
