...
import { z } from 'zod'
import { createInvoicePeriod, nextEligibleBillingDate } from '../../src/lib/billing'
...
const inputSchema = z.object({
  ...
  expectedPeriodStart: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), periodStart: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
  ...
  try {
    if (request.method === 'GET') {
      ...
      if (request.query.previewCustomerId) {
        ...
        const customer = await db.execute({ sql: `SELECT customers.name, customers.next_billing_start_date AS nextBillingStartDate, plans.name AS planName, plans.price_paise AS pricePaise,
      ...
      if (request.query.deletePreview) {
        ...
        const payments = await db.execute({ sql: `SELECT DISTINCT payments.id, payments.payment_code AS paymentCode, payments.amount_received_paise AS amountReceivedPaise,
          payments.discount_given_paise AS discountGivenPaise FROM payment_allocations JOIN payments ON payments.id = payment_allocations.payment_id
      ...
      if (request.query.id) {
        ...
        const invoice = await db.execute({ sql: `SELECT invoices.id, invoices.invoice_code AS invoiceCode, invoices.customer_id AS customerId, customers.customer_code AS customerCode,
          ...
          invoices.total_payable_paise AS totalPayablePaise, invoices.due_date AS dueDate, invoices.status, invoices.billing_mode AS billingMode, invoices.historical_reason AS historicalReason
          FROM invoices JOIN customers ON customers.id = invoices.customer_id WHERE invoices.id = ? AND invoices.service_type = ? AND invoices.is_deleted = 0`, args: [invoiceId, serviceType] })
        if (!invoice.rows[0]) return sendError(response, 404, 'Invoice not found.')
      ...
      const result = await db.execute({ sql: `SELECT invoices.id, invoices.invoice_code AS invoiceCode, invoices.customer_id AS customerId,
        ...
        invoices.issued_date AS issuedDate, invoices.total_payable_paise AS totalPayablePaise, invoices.status, invoices.due_date AS dueDate, invoices.is_merged AS isMerged, invoices.billing_mode AS billingMode,
    ...
    if (request.method === 'DELETE') {
      ...
      await withWriteTransaction(async (transaction) => {
        const invoice = await transaction.execute({ sql: `SELECT id, customer_id, period_start, billing_mode,
        ...
        if (later.rows[0] && invoice.rows[0].billing_mode !== 'historical') throw new InvoiceRequestError(409, 'Only the latest renewal invoice can be deleted. Historical correction invoices can be removed with an audit reason.')
        ...
        await recomputeBillingPosition(transaction, Number(invoice.rows[0].customer_id))
        await recordAudit(transaction, { entityType: 'invoice', entityId: invoiceId, action: 'invoice_deleted', reason, details: { linkedPaymentIds: paymentIds, restoredMergeSources: Number(invoice.rows[0].is_combined) === 1 } })
      })
