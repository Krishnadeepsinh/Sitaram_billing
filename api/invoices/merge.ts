...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    try {
      ...
      if (sources.rows.length !== ids.length) { await transaction.rollback(); return sendError(response, 404, 'One or more invoices are unavailable.') }
      const customerId = Number(sources.rows[0].customer_id)
      if (sources.rows.some((row) => Number(row.customer_id) !== customerId)) { await transaction.rollback(); return sendError(response, 400, 'Merged invoices must belong to the same customer.') }
      if (sources.rows.some((row) => Number(row.allocated) > 0)) { await transaction.rollback(); return sendError(response, 409, 'Only fully unpaid invoices can be merged.') }
      ...
      const merged = await transaction.execute({ sql: `INSERT INTO invoices (invoice_code, customer_id, service_type, customer_name_snapshot, area_name_snapshot, plan_name_snapshot, stb_number_snapshot, period_start, period_end, issued_date, months_billed, current_period_amount_paise, previous_due_snapshot_paise, total_payable_paise, due_date, created_at)
        VALUES (?, ?, ?, ?, ?, 'Multiple plans', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id`, args: [invoiceCode, customerId, input.serviceType, first.customer_name_snapshot, first.area_name_snapshot, first.stb_number_snapshot, first.period_start, last.period_end, todayInBusinessTimezone(), sources.rows.reduce((sum, row) => sum + Number(row.months_billed), 0), serviceAmount, openingAmount, serviceAmount + openingAmount, last.due_date, now] })
      ...
      return response.status(201).json({ invoiceId: mergedId, invoiceCode })
