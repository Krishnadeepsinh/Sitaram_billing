  ...
  try {
    ...
    const result = await withWriteTransaction(async (transaction) => {
      ...
      for (const customer of customers.rows) {
        ...
        if (nextBillingStartDate.slice(0, 7) !== input.throughMonth) { skipped.push({ ...identity, reason: `The next 30-day service period starts on ${nextBillingStartDate}. Select ${nextBillingStartDate.slice(0, 7)} to bill this customer.` }); continue }
        const months = 1
        if (input.preview) {
          const period = createInvoicePeriod(nextBillingStartDate, months)
    ...
    if (error instanceof z.ZodError || error instanceof DateInputError) return sendError(response, 400, 'Choose a valid billing month.')
