...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    const [billed, collected, discounts, today, outstanding, subscribers, payments, paymentCount, expenses, expenseCount, trends, quality, modeTotals, expiringSoon, areaBreakdown] = await Promise.all([
      ...
      db.execute({ sql: `SELECT COUNT(*) AS value FROM customers WHERE status = 'active' AND is_deleted = 0
        AND EXISTS (SELECT 1 FROM invoices WHERE invoices.customer_id = customers.id AND invoices.is_deleted = 0 AND invoices.is_merged = 0 AND invoices.period_start <= ? AND invoices.period_end >= ?)
        AND (? IS NULL OR service_type = ?) AND (? IS NULL OR area_id = ?)`, args: [todayInBusinessTimezone(), todayInBusinessTimezone(), service, service, areaId, areaId] }),
